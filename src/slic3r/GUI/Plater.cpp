...

    {
        ...
        p->m_flushing_volume_btn->Bind(
            wxEVT_BUTTON, ([parent](wxCommandEvent& e) {
                ...
                WipingDialog dlg(parent, cast<float>(init_matrix), cast<float>(init_extruders), extruder_colours, extra_flush_volume,
                                 flush_multiplier);
...
Plater::priv::priv(Plater* q, MainFrame* main_frame)
    ...
    , config(Slic3r::DynamicPrintConfig::new_from_defaults_keys(
          {"printable_area", "bed_exclude_area", "bed_custom_texture", "bed_custom_model", "print_sequence", "extruder_clearance_radius",
           "extruder_clearance_height_to_lid", "extruder_clearance_height_to_rod", "skirt_loops", "skirt_speed", "skirt_distance",
           ...
           "wipe_tower_cone_angle", "wipe_tower_extra_spacing", "wipe_tower_extruder", "best_object_pos"}))
        ...
        try {
            if (type_3mf) {
                ...
                model = Slic3r::Model::read_from_file(
                    ...
                    [this, &dlg, real_filename, &progress_percent, &file_percent, INPUT_FILES_RATIO, total_files, i,
                     &designer_model_id](int current, int total, bool& cancel, std::string& mode_id) {
...
wxString Plater::priv::get_export_file(GUI::FileType file_type)
{
    ...
    wxFileDialog dlg(q, dlg_title,
                     ...
                     from_path(uuid_str), wildcard, wxFD_SAVE | wxFD_OVERWRITE_PROMPT | wxPD_APP_MODAL);
    ...
    if (path.extension() != output_file.extension()) {
        ...
        if (boost::filesystem::exists(into_u8(out_path), ec)) {
            auto result = MessageBox(q->GetHandle(),
                                     ...
                                     _L("Comfirm Save As"), MB_YESNO | MB_ICONWARNING);
            if (result != IDYES)
                return wxEmptyString;
        }
    }
...
/**
 * @brief Export gcode to file automatically without prompting the user for a file path.
*/
std::string Plater::export_gcode_path(bool prefer_removable)
{
    fs::path default_output_file;
    try {
        unsigned int state = this->p->update_restart_background_process(false, false);
    ...
    // Set a default path and filename
    std::string default_dir = std::filesystem::current_path().string();
    boost::uuids::random_generator gen;
    boost::uuids::uuid uuid = gen();
    std::string filename = boost::uuids::to_string(uuid) + default_output_file.extension().string();
    fs::path output_path = fs::path(default_dir) / filename;

    // Save the file
    this->p->export_gcode(output_path, false);  // Assuming not on removable media
...
void Plater::export_gcode_3mf(bool export_all)
{
    ...
    fs::path output_path;
    {
        ...
        wxFileDialog dlg(this, _L("Save Sliced file as:"), start_dir, from_path(default_output_file.filename()),
                         GUI::file_wildcards(FT_3MF, ext), wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
        if (dlg.ShowModal() == wxID_OK) {
            output_path = into_path(dlg.GetPath());
...
std::string Plater::export_core_3mf_path()
{
    // Get the current path as the default directory
    std::string default_dir = std::filesystem::current_path().string();

    // Generate a unique filename using UUID
    boost::uuids::random_generator gen;
    boost::uuids::uuid             uuid     = gen();
    std::string                    filename = boost::uuids::to_string(uuid) + ".3mf";

    // Combine the default directory path and the filename
    std::filesystem::path default_output_path = std::filesystem::path(default_dir) / filename;

    // Convert the path to a string and prepare it for export
    std::string path_u8 = default_output_path.string();
    wxLogMessage("export_core_3mf_path: %s", path_u8.c_str());

    // Export the file without user interaction
    export_3mf(path_u8, SaveStrategy::Silence);
...
int Plater::upload_3mf_c3dp() { return 0; }
